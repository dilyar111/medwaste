...
function Profile() {
  ...
  const [phone, setPhone]     = useState({ number: "+7705*******", code: "", sent: false, verified: false });
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      try {
        ...
        if (!active) return;
        setProfile({
          username: res.data.fullName || username,
          email: res.data.email || email,
          department: res.data.department || "",
        });
        setRole(res.data.role || "personnel");
      } catch (err) {
        console.error(err);
      } finally {
  ...
  const handlePasswordSave = async (e) => {
    ...
    try {
      await changePassword(security.current, security.newPw);
      showToast("✓ Password changed successfully");
      setSecurity({ current: "", newPw: "", confirm: "" });
    } catch (err) {
      const message = err?.response?.data?.error || err?.message || 'Unable to change password';
      showToast(`✕ ${message}`);
      console.error(err);
    }
  };
              ...
              <div className="pf-avatar-row">
                ...
                <div className="pf-avatar-info">
                  ...
                  <div className="pf-avatar-badges">
                    <span className="pf-badge pf-badge-blue">Role: {role}</span>
