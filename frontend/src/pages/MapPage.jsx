  ...
  useEffect(() => {
    ...
    const loadBins = async () => {
      try {
        ...
        if (!active) return;
        const loaded = res.data.map((bin) => ({
          id: bin._id || bin.id,
          _id: bin._id || bin.id,
          fullness: Number(bin.fullness || 0),
          status: bin.status || 'active',
          lat: Number(bin.lat) || 51.1283,
          lng: Number(bin.lng) || 71.4305,
          location: bin.location || `Container ${bin._id || bin.id}`,
          updated: bin.updated || (bin.timestamp ? new Date(bin.timestamp).toLocaleDateString('en-US', {
            month: 'short', day: 'numeric', year: 'numeric',
          }) : 'Unknown'),
        }));
        if (loaded.length) setBins(loaded);
      } catch (err) {
