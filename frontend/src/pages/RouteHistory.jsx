...
const css = `
  ...
  .rh-btn-blue { background: #1A6EFF; color: #fff; border-color: #1A6EFF; }
  .rh-btn-blue:hover { background: #0F4ECC; }
...
function RouteHistory() {
  ...
  async function fetchRoutes() {
    ...
    setError(null);
    try {
      const { data } = await getRouteHistory({ period, status });
  ...
  useEffect(() => {
    fetchRoutes();
    // eslint-disable-next-line react-hooks/exhaustive-deps -- только при первом входе (all/all)
  }, []);

  useEffect(() => {
    setSelectedRoute((prev) => {
      if (!prev) return null;
      return routes.find((r) => r.id === prev.id) || null;
    });
  }, [routes]);

  const totalDistance = routes.reduce((s, r) => s + (Number(r.distance) || 0), 0);
  const totalBins     = routes.reduce((s, r) => s + (Number(r.bins) || 0), 0);
    ...
    <>
      ...
      <div className="rh-root">
        ...
        <div className="rh-kpi-grid">
          {[
            { label: "Total Routes",         val: routes.length },
            { label: "Completed",            val: routes.filter((r) => r.status === "completed").length },
            { label: "Total Distance",       val: `${totalDistance.toFixed(1)} km` },
            { label: "Containers Collected", val: totalBins },
          ].map((k) => (
                ...
                routes.map((route) => (
                  ...
                  >
                    ...
                    <div className="rh-route-meta">
                      {route.distance} km · {route.bins} bin{route.bins !== 1 ? "s" : ""}
                    </div>
              ...
              {!selectedRoute ? (
                ...
                >
                  ...
                  {selectedRoute.coordinates.map((point, i) => (
                    <Marker key={`${selectedRoute.id}-${i}`} position={point}>
